...
if(is.null(folder_string)){
  ...
  dir.create(path = file.path(results_folder,"temp",folder_string),recursive = T)
  dir.create(path = file.path(results_folder,"full",folder_string),recursive = T)
  ...
  for (i in 1:length(INLA.max.edge.vec)){
    ...
    for (j in 1:length(true.field.variance.vec)){
      ...
      for(u in 1:length(kappa.true.vec)){
        ...
        data.table::fwrite(res.dt.tmp,file.path(results_folder,"temp",folder_string,paste0("res.dt.tmp.i_",i,"__j_",j,"__l_",l,"__u_",u,"_.csv")),append=T)
...
fwrite(res.dt,file.path(results_folder,"common.full.res.dt.csv"),append = T)
...
fwrite(x = summary.RMSE.res.dt,file.path(results_folder,"summary",folder_string,"summary.RMSE.res.dt.csv"))
fwrite(x = summary.MSE.res.dt,file.path(results_folder,"summary",folder_string,"summary.MSE.res.dt.csv"))
fwrite(x = summary.sqbias.res.dt,file.path(results_folder,"summary",folder_string,"summary.sqbias.res.dt.csv"))
